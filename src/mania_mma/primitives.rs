//! 逐行基元计算，对应上游 `js/patterns/primitives.js`。
//!
//! 基元是键型匹配的唯一输入：每行记录列数、重复键（jacks）、方向、长条状态等。
...
/// 行基元，对应上游 `calculatePrimitives` 的返回元素。
#[derive(Debug, Clone)]
...
pub fn calculate_primitives(chart: &Chart) -> Vec<Primitive> {
    ...
    for (offset, row) in chart.notes.iter().skip(1).enumerate() {
        ...
        // 相邻行的间隔换算成 1/4 拍时值，必须用更新前的上一行时间。
...
/// 对应上游 `lnPercent`：短按与长按头的比例，长按尾不重复计数。
