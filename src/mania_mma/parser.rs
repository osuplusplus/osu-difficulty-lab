//! `.osu` mania 解析，对应上游 `js/parser/patternOsuParser.js`。
//!
//! 解析结果决定后续所有键型判定，因此这里逐行对齐上游行为：堆叠物件保持原样、
//! 长条的 body/tail 行由跨列释放时间推导、时间一律向下取整。

use anyhow::{Result, bail};

use super::chart::{BpmData, Chart, NoteType, Row, TimeItem};

/// 打击物件，对应上游 `kind: "HitCircle" | "Hold"`。
enum HitObject {
    Circle { x: f64, time: f64 },
    Hold { x: f64, time: f64, end_time: f64 },
}

/// 时间点，对应上游 `kind: "Uninherited" | "Inherited"`。
enum TimingPoint {
    Uninherited {
        time: f64,
        ms_per_beat: f64,
...
/// 把打击物件折叠成按时间排列的行，对应上游 `convertHitObjects`。
...
/// 每种 BPM 的总时长，对应上游 `findBpmDurations`。
...
/// 解析 mania `.osu` 文本，对应上游 `parseOsuManiaFromText`。
