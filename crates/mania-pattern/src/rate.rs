//! 倍率变换，对应 OPP 侧原型 `transformRate`。
//!
//! DT/HT 的特征必须按实际时钟倍率重新分析，而不是把 NoMod 特征乘一个系数。上游把谱面
//! 文本整体缩放到目标倍率后再解析，这里保持同样的顺序：先改文本、再解析，避免时间取整
//! 顺序不同带来的差异。
...
/// JavaScript `Number` 转字符串的等价实现（覆盖本模块用到的数值范围）。
...
/// 对应上游 `transformRate`：把谱面整体缩放到目标倍率。
