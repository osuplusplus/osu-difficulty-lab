//! 键型定义与匹配，对应上游 `js/patterns/patternsDef.js` 与 `js/patterns/findPatterns.js`。
//!
//! 匹配器只读取窗口头部有限的行（最多 8 行），因此这里用固定长度窗口切片而不是整段复制。
...
/// 匹配结果，对应上游 `appendFoundPattern` 产出的对象。
#[derive(Debug, Clone)]
...
/// 把一行转换成「只看长条头」的行，对应上游 `asHeadPointRow`。
...
// 上游 `patternsDef.js` 导出了这两个匹配器，但 4K 细分表没有引用它们；保留以便与上游逐条对照。
#[allow(dead_code)]
...
/// 对应上游 `reorderSpecific`：按偏好顺序稳定排序，未列出的名称排在最后。
fn reorder_specific(
